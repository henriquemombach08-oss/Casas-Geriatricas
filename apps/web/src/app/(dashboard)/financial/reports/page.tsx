    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Visão geral consolidada</p>
        </div>
      ...
      {summary && (
        <div className="space-y-6">
          ...
          <div className="card space-y-4">
            <h2 className="font-bold text-stone-900">Indicadores do Mês</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              {[
                ...
                <div key={kpi.label} className="border rounded-lg p-3">
                  ...
                  <p className="font-bold text-stone-900 text-lg mt-0.5">{kpi.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {summary.top_debtors.length > 0 && (
            <div className="card space-y-3">
              <h2 className="font-bold text-stone-900">Inadimplência</h2>
                ...
                <tbody>
                  {summary.top_debtors.map((d, i) => (
                    <tr key={i} className="border-b last:border-0">
                      <td className="py-2 text-stone-900">{d.resident_name}</td>
          ...
          <div className="card space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="border rounded-lg p-3">
                <p className="text-stone-500 text-xs">Este mês</p>
                <p className="font-bold text-xl text-stone-900">{formatCurrency(summary.cash_flow.this_month)}</p>
              </div>
              <div className="border rounded-lg p-3">
                <p className="text-stone-500 text-xs">Mês passado</p>
                <p className="font-bold text-xl text-stone-900">{formatCurrency(summary.cash_flow.last_month)}</p>
              </div>
            </div>
